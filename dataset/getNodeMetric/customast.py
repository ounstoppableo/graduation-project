...
def parse_file(filename,improved=False):
    global enc, lines
    ...
    # remove BOM
    lines = re.sub(b'\xef\xbb\xbf', b' ', lines)
    ...
    if enc_len > 0:
        try:
            lines = re.sub(r'#.*coding\s*[:=]\s*[\w\d\-]+',  '#' + ' ' * (enc_len-1), lines)
        except:
            lines = re.sub(b'#.*coding\s*[:=]\s*[\w\d\-]+',  b'#' + b' ' * (enc_len-1), lines)
    f.close()
