...
def make_file_group(tree, filename, extension,source,raw_source_dir=''):
    """
    ...
    source = source.split('\\')[len(raw_source_dir.split('\\'))-1:]
    source[-1] = source[-1][:-3]
    token = '.'.join(source)
...
def map_it(sources, extension, no_trimming, exclude_namespaces, exclude_functions,
           ...
           skip_parse_errors, lang_params,raw_source_dir=''):
    '''
    ...
    for source, file_ast_tree in file_ast_trees:
        file_group = make_file_group(file_ast_tree, source, extension,source,raw_source_dir=raw_source_dir)
...
def code2flow(raw_source_paths, output_file, language=None, hide_legend=True,
              ...
              lang_params=None, subset_params=None, level=logging.INFO,raw_source_dir=''):
    """
    ...
    file_groups, all_nodes, edges = map_it(sources, language, no_trimming,
                                           ...
                                           include_only_namespaces, include_only_functions,
                                           skip_parse_errors, lang_params,raw_source_dir=raw_source_dir)
